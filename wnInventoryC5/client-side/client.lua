...
vSERVER = Tunnel.getInterface("wnInventoryC5")
-----------------------------------------------------------------------------------------------------------------------------------------
-- VARIABLES
-----------------------------------------------------------------------------------------------------------------------------------------
local Drops = {}
...
-----------------------------------------------------------------------------------------------------------------------------------------
-- wnInventoryC5BUTTONS
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent("wnInventoryC5:Buttons")
AddEventHandler("wnInventoryC5:Buttons",function(status)
...
-----------------------------------------------------------------------------------------------------------------------------------------
local currentWeapon = ""
RegisterNetEvent("wnInventoryC5:throwableWeapons")
AddEventHandler("wnInventoryC5:throwableWeapons",function(weaponName)
...
-- wnInventoryC5CLOSE
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent("wnInventoryC5:Close")
AddEventHandler("wnInventoryC5:Close",function()
	if Backpack then
		Backpack = false
...
RegisterNUICallback("invClose",function()
	TriggerEvent("wnInventoryC5:Close")
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- CRAFT
-----------------------------------------------------------------------------------------------------------------------------------------
...
RegisterNUICallback("Deliver",function(data)
	if MumbleIsConnected() then
		TriggerServerEvent("wnInventoryC5:Deliver",data["slot"],data["amount"])
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- USEITEM
...
RegisterNUICallback("useItem",function(data)
	if MumbleIsConnected() then
		TriggerServerEvent("wnInventoryC5:useItem",data["slot"],data["amount"])
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- SENDITEM
...
RegisterNUICallback("sendItem",function(data)
	if MumbleIsConnected() then
		TriggerServerEvent("wnInventoryC5:sendItem",data["slot"],data["amount"])
	end
end)

RegisterNUICallback("lixoItem",function(data)
	if MumbleIsConnected() then
		TriggerServerEvent("wnInventoryC5:lixoItem",data["item"],data["amount"],data["slot"])
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- UPDATESLOT
...
-- wnInventoryC5UPDATE
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent("wnInventoryC5:Update")
AddEventHandler("wnInventoryC5:Update",function(action)
	if MumbleIsConnected() then
...
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent("wnInventoryC5:clearWeapons")
AddEventHandler("wnInventoryC5:clearWeapons",function()
	if Weapon ~= "" then
		Weapon = ""
		weaponActive = false
...
-----------------------------------------------------------------------------------------------------------------------------------------
-- wnInventoryC5VERIFYWEAPON
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent("wnInventoryC5:verifyWeapon")
AddEventHandler("wnInventoryC5:verifyWeapon",function(splitName)
...
-----------------------------------------------------------------------------------------------------------------------------------------
-- wnInventoryC5PREVENTWEAPON
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent("wnInventoryC5:preventWeapon")
AddEventHandler("wnInventoryC5:preventWeapon",function(storeWeapons)
	if Weapon ~= "" then
		local ped = PlayerPedId()
		local weaponAmmo = GetAmmoInPedWeapon(ped,Weapon)
...
-- REPAIRVEHICLE
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent("wnInventoryC5:repairVehicle")
AddEventHandler("wnInventoryC5:repairVehicle",function(vehIndex,vehPlate)
...
-----------------------------------------------------------------------------------------------------------------------------------------
-- wnInventoryC5REPAIRTYRE
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent("wnInventoryC5:repairTyre")
AddEventHandler("wnInventoryC5:repairTyre",function(Vehicle,Tyres,vehPlate)
...
-- REPAIRPLAYER
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent("wnInventoryC5:repairPlayer")
AddEventHandler("wnInventoryC5:repairPlayer",function(vehIndex,vehPlate)
	if NetworkDoesNetworkIdExist(vehIndex) then
		local Vehicle = NetToEnt(vehIndex)
		if DoesEntityExist(Vehicle) then
			if GetVehicleNumberPlateText(Vehicle) == vehPlate then
...
-- REPAIRADMIN
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent("wnInventoryC5:repairAdmin")
AddEventHandler("wnInventoryC5:repairAdmin",function(vehIndex,vehPlate)
	if NetworkDoesNetworkIdExist(vehIndex) then
		local Vehicle = NetToEnt(vehIndex)
		if DoesEntityExist(Vehicle) then
			if GetVehicleNumberPlateText(Vehicle) == vehPlate then
...
-- VEHICLEALARM
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent("wnInventoryC5:vehicleAlarm")
AddEventHandler("wnInventoryC5:vehicleAlarm",function(vehIndex,vehPlate)
	if NetworkDoesNetworkIdExist(vehIndex) then
		local Vehicle = NetToEnt(vehIndex)
		if DoesEntityExist(Vehicle) then
			if GetVehicleNumberPlateText(Vehicle) == vehPlate then
...
-----------------------------------------------------------------------------------------------------------------------------------------
-- wnInventoryC5STEALTRUNK
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent("wnInventoryC5:stealTrunk")
AddEventHandler("wnInventoryC5:stealTrunk",function(entity)
...
-- FIRECRACKER
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent("wnInventoryC5:Firecracker")
AddEventHandler("wnInventoryC5:Firecracker",function()
...
RegisterNUICallback("dropItem",function(data)
	if MumbleIsConnected() then
		...
		TriggerServerEvent("wnInventoryC5:Drops",data["item"],data["slot"],data["amount"],coords["x"],coords["y"],cdz)
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- DROPS:TABLE
...
RegisterNUICallback("pickupItem",function(data)
	if MumbleIsConnected() then
		TriggerServerEvent("wnInventoryC5:Pickup",data["id"],data["amount"],data["target"])
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- WHEELCHAIR
...
-- SCANNERCOORDS
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent("wnInventoryC5:updateScanner")
AddEventHandler("wnInventoryC5:updateScanner",function(status)
	userScanner = status
	soundScanner = 999
end)
-----------------------------------------------------------------------------------------------------------------------------------------
					...
					if distance <= 7.25 then
						...
						if distance <= 1.25 then
							...
							if IsControlJustPressed(1,38) and MumbleIsConnected() then
								TriggerServerEvent("wnInventoryC5:makeProducts",{},"scanner")
...
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent("wnInventoryC5:removeTyres")
AddEventHandler("wnInventoryC5:removeTyres",function(Entity)
			...
			for k,Tyre in pairs(tyreList) do
				...
				if Selected ~= -1 then
					...
					if distance <= 1.0 then
						TriggerServerEvent("wnInventoryC5:removeTyres",Entity,Tyre)
					end
				end
			end
		else
...
-----------------------------------------------------------------------------------------------------------------------------------------
-- wnInventoryC5EXPLODETYRES
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent("wnInventoryC5:explodeTyres")
AddEventHandler("wnInventoryC5:explodeTyres",function(vehNet,vehPlate,Tyre)
