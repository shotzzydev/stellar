...
-----------------------------------------------------------------------------------------------------------------------------------------
cRP = {}
Tunnel.bindInterface("wnInventoryC5",cRP)
vPLAYER = Tunnel.getInterface("player")
vGARAGE = Tunnel.getInterface("wnGarages")
vTASKBAR = Tunnel.getInterface("taskbar")
vDELIVER = Tunnel.getInterface("deliver")
vCLIENT = Tunnel.getInterface("wnInventoryC5")
-----------------------------------------------------------------------------------------------------------------------------------------
-- VARIABLES
-----------------------------------------------------------------------------------------------------------------------------------------
local Drops = {}
...
vRP._prepare("wnInventoryC5/jesterInstagram", "SELECT * FROM smartphone_instagram WHERE user_id = @user_id")

local empregos_list = {
	[1] = { nome = "Motoclub", tag = "Motoclub" },
	[2] = { nome = "Admin", tag = "Staff" },
}

cRP.Identidade = function()
	local source = source
	...
	if user_id then
		...
		local item_inv = true
		local pegar_item = vRP.getInventoryItemAmount(user_id,"identity")
		if pegar_item[1] >= 1 then
			item_inv = true
		end

		local infos = vRP.execute("wnInventoryC5/jesterInstagram", {user_id = parseInt(user_id)})
		if infos[1] then
			return user_id,item_inv,""..identity.name.." "..identity.name2.."",bloodTypes(identity.blood),premium,kdsak,identity.phone,vRP.userGemstone(identity["steam"]),parseFormat(identity.bank),infos[1].avatarURL
		else
			return user_id,item_inv,""..identity.name.." "..identity.name2.."",bloodTypes(identity.blood),premium,kdsak,identity.phone,vRP.userGemstone(identity["steam"]),parseFormat(identity.bank),"https://cdn.discordapp.com/attachments/452891038349262849/959382742624260136/unknown.png"
		end
	end
end
...
local tableList = {
	["tablecoke"] = {
		...
		[1] = { ["timer"] = 5, ["need"] = "sulfuric", ["needAmount"] = 2 },
		[2] = { ["timer"] = 5, ["need"] = "cokeleaf", ["needAmount"] = 2 },
		[3] = { ["timer"] = 5, ["item"] = "cocaine", ["itemAmount"] = 3 }
	},
	["tableesctasy"] = {
		["anim"] = { "anim@amb@business@coc@coc_unpack_cut@","fullcut_cycle_v6_cokecutter" },
		[1] = { ["timer"] = 5, ["need"] = "amphetamine", ["needAmount"] = 2 },
		[2] = { ["timer"] = 5, ["need"] = "sulfuric", ["needAmount"] = 2 },
		[3] = { ["timer"] = 5, ["item"] = "ecstasy", ["itemAmount"] = 3 }
	},
	["tablelean"] = {
		["anim"] = { "anim@amb@business@coc@coc_unpack_cut@","fullcut_cycle_v6_cokecutter" },
		[1] = { ["timer"] = 5, ["need"] = "glass", ["needAmount"] = 2 },
		[2] = { ["timer"] = 5, ["need"] = "codeine", ["needAmount"] = 2 },
		[3] = { ["timer"] = 5, ["item"] = "lean", ["itemAmount"] = 3 }
	},
	["tablemeth"] = {
		["anim"] = { "anim@amb@business@coc@coc_unpack_cut@","fullcut_cycle_v6_cokecutter" },
		[1] = { ["timer"] = 5, ["need"] = "saline", ["needAmount"] = 2 },
		[2] = { ["timer"] = 5, ["need"] = "acetone", ["needAmount"] = 2 },
		[3] = { ["timer"] = 5, ["item"] = "meth", ["itemAmount"] = 3 }
	},
	["tableweed"] = {
		["anim"] = { "anim@amb@business@coc@coc_unpack_cut@","fullcut_cycle_v6_cokecutter" },
		[1] = { ["timer"] = 5, ["need"] = "silk", ["needAmount"] = 2 },
		[2] = { ["timer"] = 5, ["need"] = "weedleaf", ["needAmount"] = 2 },
		[3] = { ["timer"] = 5, ["item"] = "lsd", ["itemAmount"] = 3 }
	},
	["foodJuice"] = {
...
function cRP.invUpdate(Slot,Target,Amount)
	...
	if user_id then
		if vRP.invUpdate(user_id,Slot,Target,Amount) then
			TriggerClientEvent("wnInventoryC5:Update",source,"updateMochila")
		end
	end
end
-----------------------------------------------------------------------------------------------------------------------------------------
-- wnInventoryC5BADGES
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterServerEvent("wnInventoryC5:Badges")
AddEventHandler("wnInventoryC5:Badges",function(x,y,z)
	local source = source
	local user_id = vRP.getUserId(source)
	if user_id then
		local Inventory = vRP.userInventory(user_id)
...
-- wnInventoryC5DROPS
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterServerEvent("wnInventoryC5:Drops")
AddEventHandler("wnInventoryC5:Drops",function(Item,Slot,Amount,x,y,z)
	local source = source
	local Slot = tostring(Slot)
	local user_id = vRP.getUserId(source)
	if user_id then
		if Active[user_id] == nil and not vPLAYER.getHandcuff(source) and not exports["wnHud"]:Wanted(user_id) and not vRPC.inVehicle(source) then
...
RegisterServerEvent("wnInventoryC5:lixoItem")
AddEventHandler("wnInventoryC5:lixoItem",function(Item,Amount,Slot)
	local source = source
	local Slot = tostring(Slot)
	local user_id = vRP.getUserId(source)
	if user_id then
			...
			if itemDurability(Item) then
				...
				if vRP.checkBroken(Item) then
					...
					if vRP.tryGetInventoryItem(user_id,Item,Amount,true,Slot) then
						...
						TriggerClientEvent("Notify",source,"vermelho","<b>"..itemName(Item).."</b> foi jogado na Lixeira.",5000)
						TriggerClientEvent("wnInventoryC5:Update",source,"updateMochila")
					end
				end
			end
...
-- wnInventoryC5PICKUP
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterServerEvent("wnInventoryC5:Pickup")
AddEventHandler("wnInventoryC5:Pickup",function(Number,Amount,Slot)
	local source = source
	local Slot = tostring(Slot)
...
-----------------------------------------------------------------------------------------------------------------------------------------
-- wnInventoryC5SENDITEM
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterServerEvent("wnInventoryC5:sendItem")
AddEventHandler("wnInventoryC5:sendItem",function(Slot,Amount)
	local source = source
	local Slot = tostring(Slot)
				...
				if not vRP.checkMaxItens(nuser_id,Item,Amount) then
					if (vRP.inventoryWeight(nuser_id) + itemWeight(Item) * Amount) <= vRP.getWeight(nuser_id) then
						if vRP.tryGetInventoryItem(user_id,Item,Amount,true,Slot) then
							...
							TriggerClientEvent("wnInventoryC5:Update",source,"updateMochila")
							TriggerClientEvent("wnInventoryC5:Update",Player,"updateMochila")
...
-----------------------------------------------------------------------------------------------------------------------------------------
-- wnInventoryC5DELIVER
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterServerEvent("wnInventoryC5:Deliver")
AddEventHandler("wnInventoryC5:Deliver",function(Slot,Amount)
	local source = source
	local Slot = tostring(Slot)
	local Amount = parseInt(Amount)
	local user_id = vRP.getUserId(source)
...
-----------------------------------------------------------------------------------------------------------------------------------------
-- wnInventoryC5USEITEM
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterServerEvent("wnInventoryC5:useItem")
AddEventHandler("wnInventoryC5:useItem",function(Slot,Amount)
	local source = source
	local Slot = tostring(Slot)
	local Amount = parseInt(Amount)
	local user_id = vRP.getUserId(source)
	if user_id and Active[user_id] == nil then
		...
		if itemType(totalName) == "Throwing" then
			...
			if checkWeapon then
				...
				if vCLIENT.putWeaponHands(source,nameItem,1) then
					...
					TriggerClientEvent("wnInventoryC5:throwableWeapons",source,nameItem)
				end
			end
		return end
		...
		if itemType(totalName) == "Usável" or itemType(totalName) == "Animal" then
			...
			if nameItem == "contract"..string.sub(totalName,9,10) then
				TriggerClientEvent("wnInventoryC5:Close",source)
			...
			if string.sub(nameItem,1,5) == "badge" then
				if openIdentity[user_id] then
					...
					if identity then
						...
						TriggerClientEvent("vRP:Identity",source,{ mode = "fidentity", nome = identity["name"].." "..identity["name2"], nacionalidade = identity["locate"], passaporte = identity["id"], sangue = bloodTypes(identity["blood"]) })
					end
				end
			return end
			...
			if nameItem == "adrenaline" then
				...
				--if parseInt(#parAmount) > 0 then
					--TriggerClientEvent("Notify",source,"vermelho","Existem paramedicos em serviço!",5000)
					--return
				--end
					...
					if nuser_id then
						...
						if n_source then
							if vRP.getHealth(n_source) <= 101 then
								...
								TriggerClientEvent("wnInventoryC5:Buttons",source,true)
								...
								repeat
									if os.time() >= parseInt(Active[user_id]) then
										Active[user_id] = nil
										TriggerClientEvent("wnInventoryC5:Buttons",source,false)

										if vRP.tryGetInventoryItem(user_id,nameItem,1,true,slot) then
											...
											TriggerClientEvent("resetBleeding",n_source)
                ...
                if openIdentity[user_id] then
                    ...
                    if identity then
                        ...
                        if numberIdentity == user_id then
                            ...
                            TriggerClientEvent("vRP:Identity",source,{ mode = "identity", id = identity["id"], nome = identity["name"].." "..identity["name2"], nacionalidade = identity["locate"], veiculos = identity["garage"], gemas = vRP.userGemstone(identity["steam"]), porte = identity["port"], premium = premium, sangue = bloodTypes(identity["blood"]) })
                        else
                            TriggerClientEvent("vRP:Identity",source,{ mode = "fidentity", id = identity["id"], nome = identity["name"].." "..identity["name2"], nacionalidade = identity["locate"], porte = identity["port"], sangue = bloodTypes(identity["blood"]) })
                        end
                    end
                end
            return end
				...
				repeat
					if os.time() >= parseInt(Active[user_id]) then
						...
						if vRP.tryGetInventoryItem(user_id,totalName,1,true,Slot) then
							...
							TriggerClientEvent("setEnergetic",source,20,1.30)
						end
					end
				...
				if not vRPC.inVehicle(source) and not vCLIENT.checkCracker(source) then
					...
					repeat
						if os.time() >= parseInt(Active[user_id]) then
							...
							if vRP.tryGetInventoryItem(user_id,totalName,1,true,Slot) then
								TriggerClientEvent("wnInventoryC5:Firecracker",source)
							end
						end
					...
					if vehicle then
						...
						if vTASKBAR.taskMechanic(source) then
							...
							if vRP.tryGetInventoryItem(user_id,totalName,1,true,Slot) then
								...
								for _,v in ipairs(activePlayers) do
									async(function()
										TriggerClientEvent("wnInventoryC5:repairVehicle",v,vehNet,vehPlate)
									end)
								end
						...
						if vRPC.inVehicle(source) then
							...
							if vTASKBAR.taskLockpick(source) then
								if math.random(100) >= 0 then
									...
									PerformHttpRequest("https://discord.com/api/webhooks/1121093581369790526/CPRyV9Ueq8HGyRgXexKjgotNxI8-CZq3Qccnux0oH38i-BHbX8xtMn0ZdLdPntTvrVIl", function(err, text, headers) end, 'POST', json.encode({
									embeds = {
										{     
											title = "**Lockpick**",
											fields = {
								...
								if math.random(100) >= 100 then
									...
									for _,v in ipairs(activePlayers) do
										async(function()
											TriggerClientEvent("wnInventoryC5:vehicleAlarm",v,vehNet,vehPlate)
										end)
									end
									...
									for k,v in pairs(policeResult) do
										async(function()
											TriggerClientEvent("NotifyPush",v,{ code = 31, imagem = "lockpick", title = "Roubo de Veículo", criminal = "Um roubo ao "..vehicleName(vehName).." - "..vehPlate.." foi denunciado, va ate o local e efetue a prisao.", x = coords["x"], y = coords["y"], z = coords["z"], vehicle = vehicleName(vehName).." - "..vehPlate, time = os.date("%H:%M"), blipColor = 44 })
										end)
									end
								end
							end
			...
			if nameItem == "cardapio" then
				TriggerClientEvent("wnInventoryC5:Close",source)
				TriggerClientEvent("wnCardapio2:Abrir",source)
			return end
			...
			if nameItem == "drinkfest" then
				vRPC.stopActived(source)
				Active[user_id] = os.time() + 10
				TriggerClientEvent("Progress",source,10000)
				TriggerClientEvent("wnInventoryC5:Close",source)
				TriggerClientEvent("wnInventoryC5:Buttons",source,true)
				vRPC.createObjects(source,"amb@world_human_drinking@beer@male@idle_a","idle_a","prop_plastic_cup_02",49,28422,0.0,0.0,-0.10,0.0,0.0,0.0)
			...
			if nameItem == "scanner" then
				...
				TriggerClientEvent("wnInventoryC5:Close",source)
				TriggerClientEvent("wnInventoryC5:Buttons",source,true)
				TriggerClientEvent("wnInventoryC5:updateScanner",source,true)
			...
			if nameItem == "orangejuice" or nameItem == "passionjuice" or nameItem == "tangejuice" or nameItem == "grapejuice" or nameItem == "strawberryjuice" or nameItem == "bananajuice" then
				...
				TriggerClientEvent("wnInventoryC5:Close",source)
				TriggerClientEvent("wnInventoryC5:Buttons",source,true)
				vRPC.createObjects(source,"mp_player_intdrink","loop_bottle","laranja_suco",49,60309,0.0,0.0,-0.06,0.0,0.0,130.0)
			...
			if nameItem == "frangofrito" or nameItem == "baldedefrango" then
				vRPC.stopActived(source)
				Active[user_id] = os.time() + 7
				TriggerClientEvent("Progress",source,7000)
				TriggerClientEvent("wnInventoryC5:Close",source)
				TriggerClientEvent("wnInventoryC5:Buttons",source,true)
				vRPC.createObjects(source,"mp_player_inteat@burger","mp_player_int_eat_burger","prop_food_cb_chips",49,18905,0.10,0.0,0.08,150.0,320.0,160.0)
				...
				repeat
					if os.time() >= parseInt(Active[user_id]) then
						...
						if vRP.tryGetInventoryItem(user_id,totalName,1,true,Slot) then
							if nameItem == "frangofrito" then
								vRP.upgradeHunger(user_id,40)
							elseif nameItem == "baldedefrango" then
								vRP.upgradeHunger(user_id,80)
							end
						end
					end					
				...
				if not exports["homes"]:checkHotel(user_id) then
					...
					if application then
						if vRP.tryGetInventoryItem(user_id,totalName,1,true,Slot) then
							...
							Objects[tostring(Number)] = { x = mathLegth(coords["x"]), y = mathLegth(coords["y"]), z = mathLegth(coords["z"]), h = heading, object = "bkr_prop_coke_table01a", item = "tableesctasy", distance = 50, mode = "1" }
							TriggerClientEvent("objects:Adicionar",-1,tostring(Number),Objects[tostring(Number)])
							TriggerClientEvent("wnInventoryC5:Close",source)
						end
					end
				...
				if not exports["homes"]:checkHotel(user_id) then
					...
					if application then
						if vRP.tryGetInventoryItem(user_id,totalName,1,true,Slot) then
							...
							Objects[tostring(Number)] = { x = mathLegth(coords["x"]), y = mathLegth(coords["y"]), z = mathLegth(coords["z"]), h = heading, object = "bkr_prop_coke_table01a", item = "tablelean", distance = 50, mode = "1" }
							TriggerClientEvent("objects:Adicionar",-1,tostring(Number),Objects[tostring(Number)])
							TriggerClientEvent("wnInventoryC5:Close",source)
						end
					end
				...
				if not exports["homes"]:checkHotel(user_id) then
					...
					if application then
						if vRP.tryGetInventoryItem(user_id,totalName,1,true,Slot) then
							...
							local identity = vRP.userIdentity(user_id)
							PerformHttpRequest("https://discord.com/api/webhooks/1121495580305272962/l5NWJncPYMaPcTuHp0cJY7FTfGmS9N6Yj9mjAQnYbF-okJOzs5LRq_rGYPMoiQQ_VvYU", function(err, text, headers) end, 'POST', json.encode({
							embeds = {
								{     
									title = "**Fogueira**",
									fields = {
			...
			if nameItem == "milkshake" or nameItem == "cappuccino" then
				...
				TriggerClientEvent("wnInventoryC5:Close",source)
				TriggerClientEvent("wnInventoryC5:Buttons",source,true)
				vRPC.createObjects(source,"amb@world_human_aa_coffee@idle_a", "idle_a","offstore_coffe",49,28422)
			...
			if nameItem == "cupcake" then
				...
				TriggerClientEvent("wnInventoryC5:Close",source)
				TriggerClientEvent("wnInventoryC5:Buttons",source,true)
				vRPC.createObjects(source,"mp_player_inteat@burger","mp_player_int_eat_burger","offstore_cupcake",49,60309)
			...
			if nameItem == "milkpessego" then
				vRPC.stopActived(source)
				Active[user_id] = os.time() + 12
				TriggerClientEvent("Progress",source,12000)
				TriggerClientEvent("wnInventoryC5:Close",source)
				TriggerClientEvent("wnInventoryC5:Buttons",source,true)
				vRPC.createObjects(source,"amb@world_human_aa_coffee@idle_a", "idle_a","p_shk_frappuccino_04",49,28422)
			...
			return end
					...
					if nameItem == "milklimao" then
				vRPC.stopActived(source)
				Active[user_id] = os.time() + 12
				TriggerClientEvent("Progress",source,12000)
				TriggerClientEvent("wnInventoryC5:Close",source)
				TriggerClientEvent("wnInventoryC5:Buttons",source,true)
				vRPC.createObjects(source,"amb@world_human_aa_coffee@idle_a", "idle_a" ,"p_shk_frappuccino_03",49,28422)
			...
			return end
					...
					if nameItem == "milkchocolate" then
				vRPC.stopActived(source)
				Active[user_id] = os.time() + 12
				TriggerClientEvent("Progress",source,12000)
				TriggerClientEvent("wnInventoryC5:Close",source)
			...
			if nameItem == "bolodecereja" then
				vRPC.stopActived(source)
				Active[user_id] = os.time() + 12
				TriggerClientEvent("Progress",source,12000)
				TriggerClientEvent("wnInventoryC5:Close",source)
				TriggerClientEvent("wnInventoryC5:Buttons",source,true)
				vRPC.createObjects(source,"amb@code_human_wander_eating_donut@male@idle_a", "idle_c","mah_scake",49,28422) 
			...
			if nameItem == "bolodemorango" then
				vRPC.stopActived(source)
				Active[user_id] = os.time() + 12
				TriggerClientEvent("Progress",source,12000)
				TriggerClientEvent("wnInventoryC5:Close",source)
				TriggerClientEvent("wnInventoryC5:Buttons",source,true)
				vRPC.createObjects(source,"amb@code_human_wander_eating_donut@male@idle_a", "idle_c","offstore_bolo",49,28422)
			...
			if nameItem == "pirulito" then
				vRPC.stopActived(source)
				Active[user_id] = os.time() + 12
				TriggerClientEvent("Progress",source,12000)
				TriggerClientEvent("wnInventoryC5:Close",source)
				TriggerClientEvent("wnInventoryC5:Buttons",source,true)
				--vRPC.createObjects(source,"mp_player_inteat@burger", "mp_player_int_eat_burger","mah_pirulito",49,60309,0.02,0,-0.05,0.0,280.0,0.0)
				--vRPC.createObjects(source,"mp_player_inteat@burger","mp_player_int_eat_burger","mah_pirulito",49,60309,0.02,0,-0.05,0.0,360.0,0.0)
			...
			return end
				...
				if nameItem == "toddy" then
					...
					TriggerClientEvent("wnInventoryC5:Buttons",source,true)
					--vRPC.createObjects(source,"amb@world_human_aa_coffee@idle_a","idle_a","Mia_cafe",49,60309,-0.05,0.02,0.03,0.0,0.0,-190.0)
					vRPC.createObjects(source,"amb@world_human_aa_coffee@idle_a","idle_a","mah_toddynho",49,28422)
				...
				if nameItem == "sucotropical" then
					...
					TriggerClientEvent("wnInventoryC5:Buttons",source,true)
					--vRPC.createObjects(source,"amb@world_human_aa_coffee@idle_a","idle_a","Mia_cafe",49,60309,-0.05,0.02,0.03,0.0,0.0,-190.0)
					vRPC.createObjects(source,"amb@world_human_aa_coffee@idle_a","idle_a","blossom_tropicalsuco",49,28422)
				...
				if nameItem == "teaf" then
					...
					TriggerClientEvent("wnInventoryC5:Buttons",source,true)
					--vRPC.createObjects(source,"amb@world_human_aa_coffee@idle_a","idle_a","Mia_cafe",49,60309,-0.05,0.02,0.03,0.0,0.0,-190.0)
					vRPC.createObjects(source,"amb@world_human_aa_coffee@idle_a","idle_a","offstore_tea",49,28422)
				...
				if nameItem == "rosquinhaf" then
					...
					TriggerClientEvent("wnInventoryC5:Buttons",source,true)
					--vRPC.createObjects(source,"amb@world_human_aa_coffee@idle_a","idle_a","Mia_cafe",49,60309,-0.05,0.02,0.03,0.0,0.0,-190.0)
					vRPC.createObjects(source,"amb@code_human_wander_eating_donut@male@idle_a","idle_c","offstore_rosquinha",49,28422)
				...
				if nameItem == "ramenf" then
					...
					TriggerClientEvent("wnInventoryC5:Buttons",source,true)
					--vRPC.createObjects(source,"amb@world_human_aa_coffee@idle_a","idle_a","Mia_cafe",49,60309,-0.05,0.02,0.03,0.0,0.0,-190.0)
					vRPC.createObjects(source,"anim@scripted@island@special_peds@pavel@hs4_pavel_ig5_caviar_p1","base_idle","bag_ramen",49,60309)
				...
				if nameItem == "bento" then
					...
					TriggerClientEvent("wnInventoryC5:Buttons",source,true)
					--vRPC.createObjects(source,"amb@world_human_aa_coffee@idle_a","idle_a","Mia_cafe",49,60309,-0.05,0.02,0.03,0.0,0.0,-190.0)
					vRPC.createObjects(source,"anim@scripted@island@special_peds@pavel@hs4_pavel_ig5_caviar_p1","base_idle","bag_bento",49,60309)
				...
				if nameItem == "pizzaf" then
					...
					TriggerClientEvent("wnInventoryC5:Buttons",source,true)
					--vRPC.createObjects(source,"amb@world_human_aa_coffee@idle_a","idle_a","Mia_cafe",49,60309,-0.05,0.02,0.03,0.0,0.0,-190.0)
					vRPC.createObjects(source,"amb@code_human_wander_eating_donut@male@idle_a","idle_c","bag_pizza",49,28422)
					...
					repeat
						if os.time() >= parseInt(Active[user_id]) then
							...
							if vRP.tryGetInventoryItem(user_id,totalName,1,true,Slot) then
								vRP.downgradeStress(user_id,20)
								vRP.upgradeThirst(user_id,40)
								TriggerClientEvent("setEnergetic",source,15,1.10)
							end
						end
				...
				if nameItem == "macarrao" then
					vRPC.stopActived(source)
					Active[user_id] = os.time() + 12
					TriggerClientEvent("Progress",source,12000)
					TriggerClientEvent("wnInventoryC5:Close",source)
					TriggerClientEvent("wnInventoryC5:Buttons",source,true)
					vRPC.createObjects(source,"mp_player_inteat@burger","mp_player_int_eat_burger","bag_macaron",49,60309,0.00,0,-0.02,0.0,280.0,160.0)
				...
				if nameItem == "sucodelaranja" then
					vRPC.stopActived(source)
					Active[user_id] = os.time() + 12
					TriggerClientEvent("Progress",source,12000)
					TriggerClientEvent("wnInventoryC5:Close",source)
					TriggerClientEvent("wnInventoryC5:Buttons",source,true)
					vRPC.createObjects(source,"amb@world_human_aa_coffee@idle_a","idle_a","laranja_suco",49,28422,0.01,0,-0.01,0.0,0.0,0.0)
				...
				if nameItem == "caixalendaria" then
					vRPC.stopActived(source)
					Active[user_id] = os.time() + 5
					TriggerClientEvent("Progress",source,5000)
					vRPC.createObjects(source,"impexp_int-0","mp_m_waremech_01_dual-0","bag_presente04",49,24817,-0.05,0.46,-0.016,-180.0,-90.0,0.0)
				...
				if nameItem == "caixauwu" then
					...
					repeat
						if os.time() >= parseInt(Active[user_id]) then
							...
							if vRP.tryGetInventoryItem(user_id,totalName,1,true,Slot) then
								TriggerClientEvent('wnPresente:Abrir',source)
							end
						end
				...
				if nameItem == "caixauwu2" then
					vRPC.stopActived(source)
					Active[user_id] = os.time() + 5
					TriggerClientEvent("Progress",source,5000)
					vRPC.createObjects(source,"impexp_int-0","mp_m_waremech_01_dual-0","bag_presente06",49,24817,-0.05,0.46,-0.016,-180.0,-90.0,0.0)
					TriggerClientEvent("wnInventoryC5:Close",source)
					repeat
						if os.time() >= parseInt(Active[user_id]) then
							Active[user_id] = nil
							...
							if vRP.tryGetInventoryItem(user_id,totalName,1,true,Slot) then
								TriggerClientEvent('wnPresente2:Abrir',source)
							end
						end
				...
				if nameItem == "bag_angel" then
					vRPC.createObjects(source,"impexp_int-0","mp_m_waremech_01_dual-0","bag_angel",49,24817,-0.05,0.44,-0.016,-180.0,-90.0,0.0)
				return end

				if nameItem == "HelloKitty" then
					vRPC.createObjects(source,"impexp_int-0","mp_m_waremech_01_dual-0","HelloKitty",49,24817,-0.05,0.40,-0.016,-180.0,-90.0,0.0)
				return end

				if nameItem == "kittyBasquete" then
					vRPC.createObjects(source,"impexp_int-0","mp_m_waremech_01_dual-0","kittyBasquete",49,24817,-0.05,0.40,-0.016,-180.0,-90.0,0.0)
				return end

				if nameItem == "KittyGold" then
					vRPC.createObjects(source,"impexp_int-0","mp_m_waremech_01_dual-0","KittyGold",49,24817,-0.05,0.44,-0.016,-180.0,-90.0,0.0)
				return end

				if nameItem == "KittyKaiju" then
					vRPC.createObjects(source,"impexp_int-0","mp_m_waremech_01_dual-0","KittyKaiju",49,24817,-0.05,0.46,-0.016,-180.0,-120.0,0.0)
				return end

				if nameItem == "KittyKuromi" then
					vRPC.createObjects(source,"impexp_int-0","mp_m_waremech_01_dual-0","KittyKuromi",49,24817,-0.05,0.44,-0.016,-180.0,-110.0,0.0)
				return end

				if nameItem == "KittyLandKaiju" then
					vRPC.createObjects(source,"impexp_int-0","mp_m_waremech_01_dual-0","KittyLandKaiju",49,24817,-0.05,0.44,-0.016,-180.0,-90.0,0.0)
				return end

				if nameItem == "KittyLiberty" then
					vRPC.createObjects(source,"impexp_int-0","mp_m_waremech_01_dual-0","KittyLiberty",49,24817,-0.05,0.44,-0.016,-180.0,-100.0,0.0)
				return end

				if nameItem == "KittyMecha" then
					vRPC.createObjects(source,"impexp_int-0","mp_m_waremech_01_dual-0","KittyMecha",49,24817,-0.05,0.44,-0.016,-180.0,-90.0,0.0)
				return end

				if nameItem == "KittySeakaiju" then
					vRPC.createObjects(source,"impexp_int-0","mp_m_waremech_01_dual-0","KittySeakaiju",49,24817,-0.05,0.44,-0.016,-180.0,-90.0,0.0)
				return end

				if nameItem == "KittySpace" then
					vRPC.createObjects(source,"impexp_int-0","mp_m_waremech_01_dual-0","KittySpace",49,24817,-0.05,0.44,-0.016,-180.0,-90.0,0.0)
				return end

				if nameItem == "MyMelody" then
					vRPC.createObjects(source,"impexp_int-0","mp_m_waremech_01_dual-0","MyMelody",49,24817,0.05,0.44,-0.016,-180.0,-90.0,0.0)
				return end

				if nameItem == "bolinhodecereja" then
					vRPC.stopActived(source)
					Active[user_id] = os.time() + 12
					TriggerClientEvent("Progress",source,12000)
					TriggerClientEvent("wnInventoryC5:Close",source)
				...
				if nameItem == "aguadecoco" then
					...
					vRPC.createObjects(source,"amb@world_human_aa_coffee@idle_a","idle_a","Mia_Aguacoco",49,28422,0.03,0,0.02,0.0,0.0,0.0)
				...
				if nameItem == "bolodemilho" then
					vRPC.stopActived(source)
					Active[user_id] = os.time() + 12
					TriggerClientEvent("Progress",source,12000)
					TriggerClientEvent("wnInventoryC5:Close",source)
					TriggerClientEvent("wnInventoryC5:Buttons",source,true)
					vRPC.createObjects(source,"mp_player_inteat@burger","mp_player_int_eat_burger","Mia_bolomilhoechocolate",49,60309,0.05,0,-0.05,0.0,280.0,0.0)
				...
				if nameItem == "cupcakedelaranja" then
					vRPC.stopActived(source)
					Active[user_id] = os.time() + 12
					TriggerClientEvent("Progress",source,12000)
					TriggerClientEvent("wnInventoryC5:Close",source)
				...
				if nameItem == "coelhodechocolate" then
					vRPC.stopActived(source)
					Active[user_id] = os.time() + 12
					TriggerClientEvent("Progress",source,12000)
					TriggerClientEvent("wnInventoryC5:Close",source)
				...
				if nameItem == "cupcakedeabóbora" then
					vRPC.stopActived(source)
					Active[user_id] = os.time() + 12
					TriggerClientEvent("Progress",source,12000)
					TriggerClientEvent("wnInventoryC5:Close",source)
				...
				if nameItem == "cupcakerecheado" then
					vRPC.stopActived(source)
					Active[user_id] = os.time() + 12
					TriggerClientEvent("Progress",source,12000)
					TriggerClientEvent("wnInventoryC5:Close",source)
				...
				if nameItem == "cupcakededefranboesa" then
					vRPC.stopActived(source)
					Active[user_id] = os.time() + 12
					TriggerClientEvent("Progress",source,12000)
					TriggerClientEvent("wnInventoryC5:Close",source)
				...
				if nameItem == "cupcakedemelancia" then
					vRPC.stopActived(source)
					Active[user_id] = os.time() + 12
					TriggerClientEvent("Progress",source,12000)
					TriggerClientEvent("wnInventoryC5:Close",source)
					TriggerClientEvent("wnInventoryC5:Buttons",source,true)
					vRPC.createObjects(source,"mp_player_inteat@burger","mp_player_int_eat_burger","Mia_cupcakedemelancia",49,60309,0.02,0,-0.05,0.0,280.0,0.0)
				...
				if nameItem == "cupcakedemorango" then
					vRPC.stopActived(source)
					Active[user_id] = os.time() + 12
					TriggerClientEvent("Progress",source,12000)
					TriggerClientEvent("wnInventoryC5:Close",source)
				...
				if nameItem == "chocolatecremoso" then
					vRPC.stopActived(source)
					Active[user_id] = os.time() + 12
					TriggerClientEvent("Progress",source,12000)
					TriggerClientEvent("wnInventoryC5:Close",source)
					TriggerClientEvent("wnInventoryC5:Buttons",source,true)
					--vRPC.createObjects(source,"amb@world_human_aa_coffee@idle_a","idle_a","Mia_chocolatecremoso",49,28422,0.02,0,-0.05,0.0,0.0,0.0)
				...
				if nameItem == "leitecomcanela" then
					vRPC.stopActived(source)
					Active[user_id] = os.time() + 12
					TriggerClientEvent("Progress",source,12000)
					TriggerClientEvent("wnInventoryC5:Close",source)
				...
				if nameItem == "sucodemelancia" then
					vRPC.stopActived(source)
					Active[user_id] = os.time() + 12
					TriggerClientEvent("Progress",source,12000)
					TriggerClientEvent("wnInventoryC5:Close",source)
			...
			if nameItem == "sanduiche" then
				vRPC.stopActived(source)
				Active[user_id] = os.time() + 12
				TriggerClientEvent("Progress",source,12000)
				TriggerClientEvent("wnInventoryC5:Close",source)
				TriggerClientEvent("wnInventoryC5:Buttons",source,true)
				vRPC.createObjects(source,"mp_player_inteat@burger", "mp_player_int_eat_burger","Mia_sanduiche",49,60309,-0.00,0.0,-0.03,90.0,310.0,0.0) -- nome = "coxinha" , dict = "amb@code_human_wander_eating_donut@male@idle_a" , anim = "idle_c" , prop = "bagdad_coxinha" , flag = 49 , hand = 28422
			...
			if nameItem == "melancia" then
				...
				TriggerClientEvent("wnInventoryC5:Close",source)
				TriggerClientEvent("wnInventoryC5:Buttons",source,true)
				vRPC.createObjects(source,"mp_player_inteat@burger", "mp_player_int_eat_burger","Mia_melancia",49,60309,-0.01,0,-0.05,0.0,280.0,-50.0) -- nome = "coxinha" , dict = "amb@code_human_wander_eating_donut@male@idle_a" , anim = "idle_c" , prop = "bagdad_coxinha" , flag = 49 , hand = 28422
			...
			if nameItem == "waffles" then
				vRPC.stopActived(source)
				Active[user_id] = os.time() + 12
				TriggerClientEvent("Progress",source,12000)
				TriggerClientEvent("wnInventoryC5:Close",source)
				TriggerClientEvent("wnInventoryC5:Buttons",source,true)
				vRPC.createObjects(source,"mp_player_inteat@burger", "mp_player_int_eat_burger","Mia_Waffles",49,60309,-0.05,0.02,0.03,0.0,0.0,-190.0) -- nome = "coxinha" , dict = "amb@code_human_wander_eating_donut@male@idle_a" , anim = "idle_c" , prop = "bagdad_coxinha" , flag = 49 , hand = 28422
			...
			if nameItem == "redvelvet" then
				vRPC.stopActived(source)
				Active[user_id] = os.time() + 12
				TriggerClientEvent("Progress",source,12000)
				TriggerClientEvent("wnInventoryC5:Close",source)
				TriggerClientEvent("wnInventoryC5:Buttons",source,true)
				vRPC.createObjects(source,"amb@code_human_wander_eating_donut@male@idle_a", "idle_a","bag_redvelvet",49,28422,0.00,-0.04,-0.02,0.0,0.0,80.0) -- nome = "coxinha" , dict = "amb@code_human_wander_eating_donut@male@idle_a" , anim = "idle_c" , prop = "bagdad_coxinha" , flag = 49 , hand = 28422
							...
							if vCLIENT.tyreHealth(source,vehNet,Tyre) ~= 1000.0 then
								...
								if vTASKBAR.taskTyre(source) then
									if vRP.tryGetInventoryItem(user_id,totalName,1,true,Slot) then
										...
										for _,v in ipairs(activePlayers) do
											async(function()
												TriggerClientEvent("wnInventoryC5:repairTyre",v,vehNet,Tyre,vehPlate)
											end)
										end
									end
								end
			...
			if nameItem == "radio" then
				...
				vRPC.stopActived(source)
				TriggerClientEvent("wnInventoryC5:Close",source)
				TriggerEvent('openRadio',source)
			return end

			if nameItem == "divingsuit" then
				TriggerClientEvent("wnHud:Diving",source)
			return end
					...
					if otherPlayer then
						if vPLAYER.getHandcuff(otherPlayer) then
							...
							if not vTASKBAR.taskDoors(otherPlayer) then
								...
								vRPC.stopAnim(source,false)
								vPLAYER.toggleHandcuff(otherPlayer)
								TriggerClientEvent("wnInventoryC5:Close",otherPlayer)
			...
			if nameItem == "hood" then
				...
				if otherPlayer and vPLAYER.getHandcuff(otherPlayer) then
					TriggerClientEvent("wnHud:toggleHood",otherPlayer)
					TriggerClientEvent("wnInventoryC5:Close",otherPlayer)
				end
			return end
				...
				if not vRPC.inVehicle(source) then
					if Carry[user_id] then
						...
						if otherPlayer and (vRP.getHealth(otherPlayer) <= 101 or vPLAYER.getHandcuff(otherPlayer)) then
							...
							TriggerClientEvent("wnInventoryC5:Close",Carry[user_id])
			...
			if nameItem == "backpack" then
                ...
                TriggerClientEvent("Progress",source,7000)
                TriggerClientEvent("wnInventoryC5:Close",source)
                TriggerClientEvent("wnInventoryC5:blockButtons",source,true)
                vRPC.playAnim(source,true,{"clothingtie","try_tie_negative_a"},true)
			...
			if nameItem == "vipmaui" then
				if not vRP.userPremium(user_id) then
					if vRP.tryGetInventoryItem(user_id,totalName,1,true,Slot) then
						TriggerClientEvent("wnInventoryC5:Update",source,"updateMochila")
						vRP.setPremiumDeluxe(user_id)
						vRP.upgradeGemstone(user_id,500,true)
						vRP.generateItem(user_id,"dollars",200000,true)
						vRP.generateItem(user_id,"premiumplate",5,true) 
						vRP.generateItem(user_id,"newgarage",6,true) 
						vRP.generateItem(user_id,"chip",1,true)
					end
				else
					if vRP.tryGetInventoryItem(user_id,totalName,1,true,Slot) then
						TriggerClientEvent("wnInventoryC5:Update",source,"updateMochila")
						vRP.upgradePremiumDeluxe(user_id)
						vRP.upgradeGemstone(user_id,500,true)
						vRP.generateItem(user_id,"dollars",200000,true)
						vRP.generateItem(user_id,"premiumplate",5,true) 
						vRP.generateItem(user_id,"newgarage",6,true) 
			...
			if nameItem == "vipadvanced" then
				if not vRP.userPremium(user_id) then
					if vRP.tryGetInventoryItem(user_id,totalName,1,true,Slot) then
						TriggerClientEvent("wnInventoryC5:Update",source,"updateMochila")
						vRP.setPremium(user_id)
						vRP.upgradeGemstone(user_id,300,true)
						vRP.generateItem(user_id,"dollars",120000,true)
						vRP.generateItem(user_id,"premiumplate",3,true) 
						vRP.generateItem(user_id,"newgarage",6,true) 
						vRP.generateItem(user_id,"chip",1,true)
					end
				else
			...
			if nameItem == "vipdiamond" then
				if not vRP.userPremium(user_id) then
					if vRP.tryGetInventoryItem(user_id,totalName,1,true,Slot) then
						TriggerClientEvent("wnInventoryC5:Update",source,"updateMochila")
						vRP.setPremium(user_id)
						vRP.upgradeGemstone(user_id,200,true)
						vRP.generateItem(user_id,"dollars",85000,true)
						vRP.generateItem(user_id,"premiumplate",2,true) 
						vRP.generateItem(user_id,"newgarage",5,true) 
						vRP.generateItem(user_id,"chip",1,true)
					end
				else
					if vRP.tryGetInventoryItem(user_id,totalName,1,true,Slot) then
			...
			if nameItem == "vipplatinum" then
				if not vRP.userPremium(user_id) then
					if vRP.tryGetInventoryItem(user_id,totalName,1,true,Slot) then
						TriggerClientEvent("wnInventoryC5:Update",source,"updateMochila")
						vRP.setPremium(user_id)
						vRP.upgradeGemstone(user_id,130,true)
						vRP.generateItem(user_id,"dollars",60000,true)
			...
			if nameItem == "vipgold" then
				if not vRP.userPremium(user_id) then
					if vRP.tryGetInventoryItem(user_id,totalName,1,true,Slot) then
						TriggerClientEvent("wnInventoryC5:Update",source,"updateMochila")
						vRP.setPremium(user_id)
						vRP.upgradeGemstone(user_id,75,true)
						vRP.generateItem(user_id,"dollars",45000,true)
			...
			if nameItem == "vipsilver" then
				if not vRP.userPremium(user_id) then
					if vRP.tryGetInventoryItem(user_id,totalName,1,true,Slot) then
						TriggerClientEvent("wnInventoryC5:Update",source,"updateMochila")
						vRP.setPremium(user_id)
						vRP.upgradeGemstone(user_id,40,true)
						vRP.generateItem(user_id,"dollars",20000,true)
				...
				if otherPlayer then
					if vPLAYER.getHandcuff(otherPlayer) then
						...
						if nuser_id then
							...
							if vRP.hasGroup(nuser_id,"pc") then
								if vRP.tryGetInventoryItem(user_id,totalName,1,true,Slot) then
									vRP.removePermission(nuser_id,"Police")
									TriggerEvent("blipsystem:serviceExit",otherPlayer)
									TriggerClientEvent("radio:outServers",otherPlayer)
									vRP.updatePermission(nuser_id,"pc","waitpc")
									TriggerClientEvent("vRP:PoliceService",otherPlayer,false)
									TriggerClientEvent("service:Label",otherPlayer,"pc","Entrar em Serviço",5000)
									TriggerClientEvent("Notify",source,"amarelo","Todas as comunicações foram retiradas.",5000)
								end
							end

							if vRP.hasGroup(nuser_id,"PM") then
								if vRP.tryGetInventoryItem(user_id,totalName,1,true,Slot) then
									vRP.removePermission(nuser_id,"Police")
									TriggerEvent("blipsystem:serviceExit",otherPlayer)
									TriggerClientEvent("radio:outServers",otherPlayer)
									vRP.updatePermission(nuser_id,"PM","waitPM")
									TriggerClientEvent("vRP:PoliceService",otherPlayer,false)
									TriggerClientEvent("service:Label",otherPlayer,"PM","Entrar em Serviço",5000)
									TriggerClientEvent("Notify",source,"amarelo","Todas as comunicações foram retiradas.",5000)
								end
							end

							if vRP.hasGroup(nuser_id,"rota") then
								if vRP.tryGetInventoryItem(user_id,totalName,1,true,Slot) then
									vRP.removePermission(nuser_id,"Police")
									TriggerEvent("blipsystem:serviceExit",otherPlayer)
									TriggerClientEvent("radio:outServers",otherPlayer)
									vRP.updatePermission(nuser_id,"rota","waitrota")
									TriggerClientEvent("vRP:PoliceService",otherPlayer,false)
									TriggerClientEvent("service:Label",otherPlayer,"rota","Entrar em Serviço",5000)
									TriggerClientEvent("Notify",source,"amarelo","Todas as comunicações foram retiradas.",5000)
								end
							end

							if vRP.hasGroup(nuser_id,"ft") then
								if vRP.tryGetInventoryItem(user_id,totalName,1,true,Slot) then
									vRP.removePermission(nuser_id,"Police")
									TriggerEvent("blipsystem:serviceExit",otherPlayer)
									TriggerClientEvent("radio:outServers",otherPlayer)
									vRP.updatePermission(nuser_id,"ft","waitft")
									TriggerClientEvent("vRP:PoliceService",otherPlayer,false)
									TriggerClientEvent("service:Label",otherPlayer,"ft","Entrar em Serviço",5000)
									TriggerClientEvent("Notify",source,"amarelo","Todas as comunicações foram retiradas.",5000)
								end
							end

							if vRP.hasGroup(nuser_id,"gcm") then
								if vRP.tryGetInventoryItem(user_id,totalName,1,true,Slot) then
									vRP.removePermission(nuser_id,"Police")
									TriggerEvent("blipsystem:serviceExit",otherPlayer)
									TriggerClientEvent("radio:outServers",otherPlayer)
									vRP.updatePermission(nuser_id,"gcm","waitgcm")
									TriggerClientEvent("vRP:PoliceService",otherPlayer,false)
									TriggerClientEvent("service:Label",otherPlayer,"gcm","Entrar em Serviço",5000)
									TriggerClientEvent("Notify",source,"amarelo","Todas as comunicações foram retiradas.",5000)
								end
							end

							if vRP.hasGroup(nuser_id,"prf") then
								if vRP.tryGetInventoryItem(user_id,totalName,1,true,Slot) then
									vRP.removePermission(nuser_id,"Police")
									TriggerEvent("blipsystem:serviceExit",otherPlayer)
									TriggerClientEvent("radio:outServers",otherPlayer)
									vRP.updatePermission(nuser_id,"prf","waitprf")
									TriggerClientEvent("vRP:PoliceService",otherPlayer,false)
									TriggerClientEvent("service:Label",otherPlayer,"prf","Entrar em Serviço",5000)
									TriggerClientEvent("Notify",source,"amarelo","Todas as comunicações foram retiradas.",5000)
								end
							end

							if vRP.hasGroup(nuser_id,"baep") then
								if vRP.tryGetInventoryItem(user_id,totalName,1,true,Slot) then
									vRP.removePermission(nuser_id,"Police")
									TriggerEvent("blipsystem:serviceExit",otherPlayer)
									TriggerClientEvent("radio:outServers",otherPlayer)
									vRP.updatePermission(nuser_id,"baep","waitbaep")
									TriggerClientEvent("vRP:PoliceService",otherPlayer,false)
									TriggerClientEvent("service:Label",otherPlayer,"baep","Entrar em Serviço",5000)
									TriggerClientEvent("Notify",source,"amarelo","Todas as comunicações foram retiradas.",5000)
								end
							end
...
-- wnInventoryC5CANCEL
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterServerEvent("wnInventoryC5:Cancel")
AddEventHandler("wnInventoryC5:Cancel",function()
	local source = source
	local user_id = vRP.getUserId(source)
	if user_id then
		if Active[user_id] ~= nil then
		...
		if Scanners[user_id] then
			TriggerClientEvent("wnInventoryC5:updateScanner",source,false)
			TriggerClientEvent("wnInventoryC5:Buttons",source,false)
			Scanners[user_id] = nil
		end
...
local stealpedsitens = {
	...
	{ item = "card01", min = 1, max = 5 },
	{ item = "card02", min = 1, max = 5 },
	{ item = "card03", min = 1, max = 5 },
	{ item = "card04", min = 1, max = 5 },
	{ item = "card05", min = 1, max = 5 }
}
-----------------------------------------------------------------------------------------------------------------------------------------
-- STEALPEDS
-----------------------------------------------------------------------------------------------------------------------------------------
...
-----------------------------------------------------------------------------------------------------------------------------------------
-- wnInventoryC5CLEARWEAPONS
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterServerEvent("wnInventoryC5:clearWeapons")
AddEventHandler("wnInventoryC5:clearWeapons",function(user_id)
	if Ammos[user_id] then
		Ammos[user_id] = {}
		Attachs[user_id] = {}
	end
...
-----------------------------------------------------------------------------------------------------------------------------------------
-- wnInventoryC5VERIFYOBJECTS
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterServerEvent("wnInventoryC5:verifyObjects")
AddEventHandler("wnInventoryC5:verifyObjects",function(Entity,Service)
	local source = source
	local user_id = vRP.getUserId(source)
	if user_id and Active[user_id] == nil then
			...
			if verifyObjects[user_id] == nil then
				...
				for k,v in pairs(Trashs[model]) do
					if #(v["coords"] - coords) <= 0.75 and os.time() <= v["timer"] then
						...
						TriggerClientEvent("Notify",source,"vermelho","Aguarde <b>"..Cooldown.."</b> segundos.",5000)
						return
					end
				end
...
-----------------------------------------------------------------------------------------------------------------------------------------
-- wnInventoryC5APPLYPLATE
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterServerEvent("wnInventoryC5:applyPlate")
AddEventHandler("wnInventoryC5:applyPlate",function(Entity)
	local source = source
	local consultItem = {}
...
-----------------------------------------------------------------------------------------------------------------------------------------
-- wnInventoryC5CHECKSTOCKADE
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterServerEvent("wnInventoryC5:checkStockade")
AddEventHandler("wnInventoryC5:checkStockade",function(Entity)
	local policeResult = vRP.numPermission("Police")
			...
			if vTASKBAR.taskHandcuff(source) then
				...
				for k,v in pairs(policeResult) do
					async(function()
						...
						TriggerClientEvent("NotifyPush",v,{ code = 20, imagem = "armored", title = "Roubo ao Carro Forte", x = coords["x"], y = coords["y"], z = coords["z"], criminal = "Um roubo a um carro forte esta sendo realizado, va ate o local rapido.", time = os.date("%H:%M"), blipColor = 16 })
					end)
				end
			end
		end
		...
		if not userPlate then
			...
			if os.time() >= Trunks[vehPlate] then
				...
				if vTASKBAR.stealTrunk(source) then
					...
					for k,v in pairs(policeResult) do
						async(function()
							TriggerClientEvent("NotifyPush",v,{ code = 31, imagem = "lockpick", title = "Roubo de Veículo", criminal = "Um roubo ao "..vehicleName(vehModels).." - "..vehPlate.." foi denunciado, va ate o local e efetue a prisao.", x = coords["x"], y = coords["y"], z = coords["z"], time = os.date("%H:%M"), blipColor = 44 })
						end)
					end
				end
			else
...
-----------------------------------------------------------------------------------------------------------------------------------------
-- wnInventoryC5ANIMALS
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterServerEvent("wnInventoryC5:Animals")
AddEventHandler("wnInventoryC5:Animals",function(Entity)
	local source = source
	local user_id = vRP.getUserId(source)
	if user_id then
...
-----------------------------------------------------------------------------------------------------------------------------------------
-- wnInventoryC5MAKEPRODUCTS
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterServerEvent("wnInventoryC5:makeProducts")
AddEventHandler("wnInventoryC5:makeProducts",function(Entity,Table)
	local source = source
	local user_id = vRP.getUserId(source)
	if user_id and Active[user_id] == nil then
			...
			if typeTable == "cemitery" then
				if not vTASKBAR.taskOne(source) then
					...
					for k,v in pairs(policeResult) do
						async(function()
							vRPC.playSound(v,"ATM_WINDOW","HUD_FRONTEND_DEFAULT_SOUNDSET")
							TriggerClientEvent("NotifyPush",v,{ code = 20, imagem = "pertences", title = "Roubo de Pertences", x = coords["x"], y = coords["y"], z = coords["z"], criminal = "Acabou de acontecer um roubo de pertences, prenda o suspeito.", time = os.date("%H:%M"), blipColor = 16 })
						end)
					end
				end
			end
...
-----------------------------------------------------------------------------------------------------------------------------------------
-- wnInventoryC5DESMANCHAR
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterServerEvent("wnInventoryC5:Desmanchar")
AddEventHandler("wnInventoryC5:Desmanchar",function(Entity)
	local source = source
	...
	if user_id and dismantleList[vehName] and Active[user_id] == nil then
		...
		repeat
			if os.time() >= parseInt(Active[user_id]) then
				...
				TriggerClientEvent("player:applyGsr",source)
				TriggerClientEvent("wnInventoryC5:Buttons",source,false)
				TriggerEvent("wnGarages:deleteVehicle",Entity[4],Entity[1])
...
-----------------------------------------------------------------------------------------------------------------------------------------
-- wnInventoryC5DISMANTLE
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterServerEvent("wnInventoryC5:Dismantle")
AddEventHandler("wnInventoryC5:Dismantle",function()
	local source = source
	local user_id = vRP.getUserId(source)
	if user_id then
...
-----------------------------------------------------------------------------------------------------------------------------------------
-- wnInventoryC5REMOVETYRES
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterServerEvent("wnInventoryC5:removeTyres")
AddEventHandler("wnInventoryC5:removeTyres",function(Entity,Tyre)
	local source = source
	local user_id = vRP.getUserId(source)
	if user_id and Active[user_id] == nil then
							...
							if os.time() >= parseInt(Active[user_id]) then
								...
								if DoesEntityExist(Vehicle) and not IsPedAPlayer(Vehicle) and GetEntityType(Vehicle) == 2 then
									if vCLIENT.tyreHealth(source,Entity[4],Tyre) == 1000.0 then
										...
										for _,v in ipairs(activePlayers) do
											async(function()
												TriggerClientEvent("wnInventoryC5:explodeTyres",v,Entity[4],Entity[1],Tyre)
											end)
										end
									end
								end
